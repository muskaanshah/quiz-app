function QuizResultQuestion() {
    return (
        <div>
            <div className="divider my-8"></div>
            <p>Question 1</p>
            <div>
                <p className="px-4 py-3 my-4 bg-skin-fillmuted rounded-md">
                    Option 1 jikcmrec vijrmdvkrdv irkmvckrv
                </p>
                <p className="px-4 py-3 my-4 bg-skin-fillmuted rounded-md">
                    Option 2 jikcmrec vijrmdvkrdv irkmvckrv
                </p>
                <p className="px-4 py-3 my-4 bg-skin-fillmuted rounded-md bg-red-500">
                    Option 3 jikcmrec vijrmdvkrdv irkmvckrv
                </p>
                <p className="px-4 py-3 my-4 bg-skin-fillmuted rounded-md bg-green-500">
                    Option 4 jikcmrec vijrmdvkrdv irkmvckrv
                </p>
            </div>
        </div>
    );
}

export { QuizResultQuestion };
